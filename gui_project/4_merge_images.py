...
from tkinter import filedialog  # 서브 모듈은 다시 작성해야 사용 가능
...
def add_file():
    files = filedialog.askopenfilenames(title="이미지 파일을 선택하세요",
                                        filetypes=(("PNG파일", "*.png"),
                                                   ("모든 파일", "*.*")),
                                        initialdir="/Users/Desktop/study/Python/")  # initialdir : 최초에 사용자가 지정한 경로를 보여줌
    # 사용자가 선택한 파일 목록
    for file in files:
...
def merge_image():
    ...
    # 최대 넓이, 전체 높이 구해옴
    max_width, total_hegiht = max(widths), sum(heights)

    # 스케치북 준비
    result_img = Image.new(
        "RGB", (max_width, total_hegiht), (255, 255, 255))  # 배경 흰색
    y_offset = 0  # y 위치
    # for img in images:
    ...
    dest_path = os.path.join(txt_dest_path.get(), "nado_potho.jpg")
...
txt_dest_path.pack(side="left", fill="x", expand=True, padx=5, pady=5,
                   ipady=4)  # ipady : 안쪽 패딩 y 길이 설정 <-> ipadx
